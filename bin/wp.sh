...
usage(){
  ...
  printf 'OPTIONS:\n'
  printf '  -g, --gui                  use yad to select a PRESET\n'
  ...
  printf '  wp.sh --gui                Launch with yad GUI\n'
  printf '\n'
...
# displaying with yad order of the PRESETs is lost.
# Example config:
'mac_day' '
...
select_preset(){
  ...
  # GUI
    ...
    if [ -x `command -v yad` ]; then
      ...
      printf "GUI ERROR: yad and zenity are unavailable. Exiting...\n"
    fi
    WP="$("${DIALOG_CMD}" --list --separator='' --hide-header \
      --hide-column=2 --print-column=2 --search-column=1 \
      --height=540 \
      --title="Change wallpaper" --text "Change wallpaper" \
      --column="Wallpaper" --column="command"  \
      -- "${LIST[@]}")"
